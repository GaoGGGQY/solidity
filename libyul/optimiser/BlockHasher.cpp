...
void BlockHasher::operator()(Block const& _block)
{
	...
	for (auto const& variableReference: subBlockPrefixHasher.m_variableReferences)
		if (variableReference.second.isExternal)
			(*this)(Identifier{{}, variableReference.first});
	hash(compileTimeLiteralHash("Block_END"));

	BlockHasher subBlockSuffixHasher(m_state);
	blockPart = BlockPart { &_block, _block.statements.size(), _block.statements.size() };
	for (auto const& statement: _block.statements)
	{
		subBlockSuffixHasher.visit(statement);
		--blockPart.start;
		m_state.blockPrefixHashes[blockPart] = subBlockSuffixHasher.m_hash;
		m_state.hashClasses[subBlockSuffixHasher.m_hash].emplace_back(blockPart);
	}
}
